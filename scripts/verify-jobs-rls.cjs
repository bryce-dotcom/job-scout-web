...
;(async () => {
  for (const t of ['jobs','job_lines','job_sections']) {
  ...
  const TEST_EMAIL = `rls-3d-${Date.now()}@hhh.services`
  ...
  try {
    ...
    aid = au.user.id
    const { data: emp } = await ADMIN.from('employees').insert({ company_id: 3, name: 'RLS 3d', email: TEST_EMAIL, role: 'Sales', user_role: 'User', active: true }).select().single()
    eid = emp.id
