...
export async function generatePaystubPdf({ paystub, employee, company, ytd }) {
  ...
  // ---- 1099 contractor: a simple payment statement (no withholding) ----
  ...
  if (isContractor) {
    ...
    set(10, 'bold')
    doc.text(String(c.company_name || c.legal_name || 'Company'), M, y)
    doc.text(String(e.w9_business_name || e.w9_legal_name || e.name || 'Contractor'), colRC, y)
    set(9, 'normal', muted)
    const fromLines = [c.address, [c.city, c.state].filter(Boolean).join(', ') + (c.zip ? ' ' + c.zip : ''), c.ein ? 'EIN ' + c.ein : null].filter(x => x && String(x).trim())
    const toLines = [(e.w9_legal_name && e.w9_business_name) ? e.w9_legal_name : null, e.home_address, [e.home_city, e.home_state].filter(Boolean).join(', ') + (e.home_zip ? ' ' + e.home_zip : '')].filter(x => x && String(x).trim())
    let yy = y + 13
    for (let i = 0; i < Math.max(fromLines.length, toLines.length); i++) {
      if (fromLines[i]) doc.text(String(fromLines[i]), M, yy)
      if (toLines[i]) doc.text(String(toLines[i]), colRC, yy)
      yy += 12
    }
    y = yy + 6
    set(9, 'normal', [40, 44, 46]); doc.text(`Service period: ${fmtDate(p.period_start)} – ${fmtDate(p.period_end)}`, M, y); y += 14
    rule(y); y += 16
    const amount = num(p.gross_pay), reimb = num(p.reimbursement_pay), total = amount + reimb
    const cCur = M + 340, cYtd = RIGHT
    set(8, 'bold', muted); doc.text('PAYMENT DETAILS', M, y); doc.text('Current', cCur, y, { align: 'right' }); doc.text('YTD', cYtd, y, { align: 'right' })
    y += 4; rule(y); y += 12
    const prow = (label, cur, ytdv, bold) => { set(9, bold ? 'bold' : 'normal', [40, 44, 46]); doc.text(label, M, y); doc.text(money(cur), cCur, y, { align: 'right' }); doc.text(money(ytdv), cYtd, y, { align: 'right' }); y += 13.5 }
    prow('Amount', amount, num(yt.gross_pay))
    if (reimb > 0 || num(yt.reimbursement_pay) > 0) prow('Reimbursements', reimb, num(yt.reimbursement_pay))
    y += 2; rule(y); y += 12
    prow('Total Payment', total, num(yt.gross_pay) + num(yt.reimbursement_pay), true)
    set(7.5, 'normal', muted)
    doc.text('1099 contractor payment — no taxes withheld. Generated by JobScout.', M, doc.internal.pageSize.getHeight() - 24)
