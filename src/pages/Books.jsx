...
import StripeTransactionsTab from './books/StripeTransactionsTab'
...
// The Tax Category dropdown (IRS Form 1065 lines) lives in lib/taxCategories
...
export default function Books() {
  ...
  const [expenseForm, setExpenseForm] = useState({
    description: '', amount: '', expense_date: '', vendor: '', category_id: '', bank_account_id: '',
  ...
  const autoLinkWalletPayouts = async () => {
    ...
    for (const { t, wallet } of walletDeposits) {
      ...
      if (pick.basis === 'net') await recordWalletFeeExpense(wallet, t, pick.total)
  ...
  // A business-profile payout arrives net of the wallet's fee. The fee never
  // touches the bank, so nothing else will ever record it: book it as an
  // expense whenever a payout is linked on a net basis.
  const recordWalletFeeExpense = async (wallet, deposit, grossTotal) => {
    const fee = Math.round((grossTotal - Math.abs(parseFloat(deposit.amount) || 0)) * 100) / 100
    if (!(fee > 0)) return
    const feeCategory = (expenseCategories || []).find(c => /fee|merchant|bank charge/i.test(c.name || ''))
    const walletAccount = (bankAccounts || []).find(b => isManualAccount(b) && walletForAccountName(b.name)?.id === wallet.id)
    const { error } = await supabase.from('manual_expenses').insert([{
      company_id: companyId,
      description: `${wallet.label} fees — payout of ${formatCurrency(grossTotal)} on ${String(deposit.date).slice(0, 10)}`,
      amount: fee,
      expense_date: String(deposit.date).slice(0, 10),
      vendor: wallet.label,
      category_id: feeCategory?.id || null,
      bank_account_id: walletAccount?.id || null,
    }])
    if (error) { toast.error(`Payout linked, but the ${wallet.label} fee expense was not recorded: ` + error.message); return }
    await fetchExpenses()
  }
  ...
  const linkWalletPayout = async () => {
    ...
    if (!e2 && walletPayout.basis === 'net') await recordWalletFeeExpense(walletPayout.wallet, deposit, walletPayout.total)
  ...
  const handleSaveExpense = async () => {
    ...
    const payload = {
      ...
      // Which account it came out of — a bank, Stripe, or a manual wallet
      // (Venmo, Cash App). Feeds the wallet balance estimate on Accounts.
      bank_account_id: expenseForm.bank_account_id ? parseInt(expenseForm.bank_account_id, 10) : null
    }
    let expenseId
    if (editingItem) {
  ...
  const walletEstimateFor = (acct) => {
    ...
    const paidSince = (expenses || [])
      .filter(e => e.bank_account_id === acct.id && String(e.expense_date || '') >= sinceDay)
      .reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0)
    if (inSince === 0 && outSince === 0 && paidSince === 0) return null
    const estimate = Math.round(((parseFloat(acct.current_balance) || 0) + inSince - outSince - paidSince) * 100) / 100
    return { wallet, inSince, outSince, paidSince, estimate }
  }
  ...
  // A Zelle deposit from a named sender who is the only customer match AND
  // has exactly one open invoice owing exactly this amount: safe to record
  // in one click, behind a confirm. Anything looser goes through the matcher.
  const zelleQuickMatch = (t) => {
    if (!t || !(parseFloat(t.amount) < 0) || t.matched_invoice_id) return null
    const sender = zelleSenderName(t)
    if (!sender) return null
    const depAmount = Math.abs(parseFloat(t.amount) || 0)
    const candidates = (invoices || [])
      .filter(i => !['Paid', 'Cancelled'].includes(i.payment_status) && i.customer?.name)
      .map(i => ({ name: i.customer.name, inv: i, open: Math.max(0, invoiceCustomerBalance(i) - (paymentsByInvoiceId.get(i.id) || 0)) }))
    const m = matchCustomerName(sender, candidates)
    if (!m) return null
    const theirs = candidates.filter(c => c.name === m.candidate.name && Math.abs(c.open - depAmount) < 0.01)
    return theirs.length === 1 ? { sender, inv: theirs[0].inv, amount: depAmount } : null
  }
  const recordZelleAsPayment = async (t) => {
    const q = zelleQuickMatch(t)
    if (!q) return
    const inv = q.inv
    if (!confirm(`Record ${formatCurrency(q.amount)} Zelle from ${q.sender} as payment in full on ${inv.invoice_id || `invoice ${inv.id}`} (${inv.customer?.name})?`)) return
    const { data: payRow, error } = await supabase.from('payments').insert([{
      ...
      notes: `Zelle from ${q.sender} (bank deposit)`,
    }]).select('id').single()
    if (error) { toast.error('Could not record the payment: ' + error.message); return }
    await supabase.from('invoices').update({ payment_status: 'Paid', updated_at: new Date().toISOString() }).eq('id', inv.id)
    await supabase.from('plaid_transactions').update({ matched_invoice_id: inv.id, matched_payment_id: payRow.id, matched_at: new Date().toISOString() }).eq('id', t.id)
    toast.success(`Recorded — ${inv.invoice_id || inv.id} is paid`)
    await Promise.all([fetchPaymentsLifetime(), fetchPlaidTransactions?.()])
  }
    ...
    <div style={{ padding: isMobile ? '16px' : '24px', maxWidth: '100%', overflowX: 'hidden' }}>
      ...
      {activeTab === 'overview' && (
        <>
          ...
          {/* Upcoming services alert — when service visits (warranty,
              annual checkup, etc.) are due in the next 30 days, prompt
              the ops user to schedule them. Otherwise these silently slip
              past their due date and we miss the chance to call the
              customer first. */}
          <UpcomingServicesAlert theme={theme} companyId={companyId} navigate={navigate} />
                    ...
                    {unmatchedDeposits.map(dep => { const payout = !!processorPayoutNote(dep.merchant_name || dep.name); return (
                      <div key={dep.id} style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {zelleQuickMatch(dep) && (
                            <button
                              onClick={() => recordZelleAsPayment(dep)}
                              style={{ marginTop: 4, padding: '4px 10px', borderRadius: 6, border: `1px solid ${theme.accent}`, backgroundColor: 'transparent', color: theme.accent, fontSize: 11, fontWeight: 600, cursor: 'pointer' }}
                            >
                              Record as {zelleQuickMatch(dep).inv.customer?.name}'s payment
                            </button>
                          )}
                          ...
                          {(() => {
                            ...
                            return (
                              <div style={{ fontSize: '11px', color: theme.textSecondary, marginTop: '4px', lineHeight: 1.5 }}>
                                ...
                                {' '}({formatCurrency(est.inSince)} recorded in{est.outSince ? `, ${formatCurrency(est.outSince)} cashed out` : ''}{est.paidSince ? `, ${formatCurrency(est.paidSince)} spent from it` : ''} since then) ·{' '}
      ...
      {activeTab === 'tax' && (
        <>
          {/* The bank rows that decide the tax number — checks tagged as
              wages, deposits nobody has matched. Hidden when there are none. */}
          <FrankieSecondLook theme={theme} isMobile={isMobile} />
          ...
          {(() => {
            ...
            return (
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(400px, 1fr))', gap: '24px' }}>
                ...
                {/* AP Summary — Accounts Payable from bills table */}
                <APSummaryCard theme={theme} companyId={companyId} statCardStyle={statCardStyle} formatCurrency={formatCurrency} navigate={navigate} />
            ...
            <div style={{ padding: '20px', maxHeight: '70vh', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                ...
                <div>
                  ...
                  <select value={expenseForm.bank_account_id} onChange={(e) => setExpenseForm({ ...expenseForm, bank_account_id: e.target.value })} style={inputStyle}>
                    <option value="">— Not tracked —</option>
                    {(bankAccounts || []).map(b => (
                      <option key={b.id} value={b.id}>{b.name}{isManualAccount(b) ? ' (manual)' : ''}</option>
                    ))}
                  </select>
                </div>
              </div>
