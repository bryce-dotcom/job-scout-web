                ...
                <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: `1px solid ${T.border}` }}>
                  ...
                  <div style={{ marginBottom: '12px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0', maxWidth: '240px' }}>
                      ...
                      <input type="number" min="1" inputMode="numeric" value={r.qty || ''} onFocus={e => e.target.select()} onChange={e => updateLine(r.id, 'qty', e.target.value === '' ? '' : (parseInt(e.target.value) || 1))} onBlur={e => { if (!e.target.value) updateLine(r.id, 'qty', 1); }} style={{ flex: 1, minWidth: 0, height: '48px', border: `2px solid ${T.border}`, borderLeft: 'none', borderRight: 'none', background: T.bgInput, color: T.text, fontSize: '22px', fontWeight: '700', textAlign: 'center', MozAppearance: 'textfield', WebkitAppearance: 'none', outline: 'none', boxSizing: 'border-box' }} />
