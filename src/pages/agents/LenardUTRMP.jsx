...
export default function LenardUTRMP() {
  ...
  const [dougieStep, setDougieStep] = useState(null); // null | 'preview' | 'loading' | 'review'
  ...
  const giveMe = useMemo(() => {
    // Frozen baseline = snapshots from before any give-me actions
    const baseline = giveMeFrozenBaseline > 0 ? giveMeFrozenBaseline : baselineProjectCost;
    const baselineIncentive = giveMeFrozenIncentive > 0 ? giveMeFrozenIncentive : rawIncentive;
    ...
    const baselineRebate = +Math.min(baselineIncentive, baselineCapAmt).toFixed(2);
    // Additional incentive captured by raising price, upgrading tiers/controls
    ...
    // Real give-me = 50% of (extra incentive from all give-me actions + additional OOP)
  ...
  }, [giveMeFrozenBaseline, giveMeFrozenIncentive, baselineProjectCost, effectiveProjectCost, rawIncentive, estimatedRebate, capPct, repAdditionalOOP]);
  ...
  const saveProject = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${SUPABASE_URL}/functions/v1/lenard-save`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${SUPABASE_ANON}` }, body: JSON.stringify({ customerName: projectName, contactName, phone: savePhone, email: saveEmail, address: saveAddress, city: saveCity, state: saveState, zip: saveZip, meterNumber: saveMeterNumber, ein: saveEIN, projectData, programType: 'ut-rmp', leadOwnerId: leadOwnerId || null, existingLeadId: savedLeadId || null, existingAuditId: savedAuditId || null, signatureData: signatureData || null }) });
      const data = await resp.json();
      if (data.success) {
                ...
                <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: `1px solid ${T.border}` }}>
                  ...
                  <div style={{ marginBottom: '12px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0', maxWidth: '240px' }}>
                      ...
                      <input type="text" inputMode="numeric" pattern="[0-9]*" value={r.qty === '' ? '' : (r.qty || 1)} onFocus={e => e.target.select()} onChange={e => { const v = e.target.value.replace(/[^0-9]/g, ''); updateLine(r.id, 'qty', v === '' ? '' : parseInt(v)); }} onBlur={e => { if (!e.target.value) updateLine(r.id, 'qty', 1); }} style={{ flex: 1, minWidth: 0, height: '48px', border: `2px solid ${T.border}`, borderLeft: 'none', borderRight: 'none', background: T.bgInput, color: T.text, fontSize: '22px', fontWeight: '700', textAlign: 'center', MozAppearance: 'textfield', WebkitAppearance: 'none', outline: 'none', boxSizing: 'border-box' }} />
