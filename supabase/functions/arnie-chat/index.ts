...
const PROPOSE_RECORD_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      value: { type: 'string', description: 'The new status, the note text, a YYYY-MM-DD date — or for shift_close the clock-out EXACTLY as the user said it ("5:30 yesterday", "Thursday at 6pm", "now") or YYYY-MM-DD HH:MM if they gave a date — never turn a weekday into a date yourself, the server does that from the user\'s calendar. For lead_merge: "" to keep the original (or older) lead, "newer" to keep the newer, or the id of the lead to keep. For section_assign: the person\'s name as said.' },
...
const PROPOSE_CREATE_TOOL = {
  ...
  input_schema: {
    type: 'object',
    properties: {
      target: { type: 'string', enum: ['lead', 'diagnosis', 'ticket', 'appointment', 'quote', 'followup', 'payment', 'memory', 'expense', 'company_setup', 'employee', 'price_book', 'won', 'schedule'], description: 'lead = a new sales lead. diagnosis = what was wrong and what fixed it, kept on the job for next time. ticket = a bug report, feature request or question for the JobScout team, filed as this person. appointment = book a sales visit on an existing lead (sets the lead to Appointment Set, hands it to the rep, and creates the setter fee — the same as booking from Lead Setter). quote = a Draft estimate for a lead or customer, lines from the price book, nothing sent. followup = a personal note from the rep on a quote that has gone quiet, SENT to the customer when they approve the card — the one draft that cannot be undone. payment = money that arrived on an open invoice (cash, check, ACH, Venmo, Zelle, PayPal, financing) — recorded against the invoice, the status updated, the receipt emailed; admin only. memory = one line about THIS user to carry into every future conversation (a nickname for a job or customer, a preference, a standing fact about them) — kept only when they approve the card. expense = money the company spent — a receipt photo read off the paper (merchant, date, total) or told in words; logged on Expenses as Pending; the photo is attached when they approve. company_setup = the whole company profile from a name, an address, a trade and an entity type — the server derives time zone, state taxes, sales tax, SUI, the first business unit, service types, NAICS and the AI crew, and the card shows every derived item with its source; owner/admin only. employee = a new person on the team (name, title, email, phone, access, pay, start day) — the row the Employees page writes plus the login invite; admin only, and pay lands only when the caller may see pay. price_book = the items read off an attached price list, photo, PDF or spreadsheet, as rows — checked and deduped on the server; manager+. won = an estimate the customer accepted ("Halifax signed", "they went with us", "mark the Halifax estimate won") — on approve the estimate reads Approved, the job is created from it with its lines, the deposit invoice if the proposal has one, and the lead moves to delivery; the rep’s own estimate or a manager’s. schedule = put a job on the calendar ("schedule the Halifax job Thursday at 8 with Jordan and Mike") — the Job Board’s Schedule modal: start and end, the Scheduled status, the crew with a calendar entry each, the lead mirrored; clashes shown on the card; manager+.' },
      fields: {
        type: 'object',
        description:
        ...
        properties: {
          ...
          name: { type: 'string', description: 'company_setup: the company name' }, trade: { type: 'string', description: 'company_setup: what they do, in their words ("commercial lighting and electrical", "lawn care")' }, entity: { type: 'string', description: 'company_setup: LLC, S-corp, C-corp, partnership or sole proprietor, as said' }, legal_name: { type: 'string' }, website: { type: 'string' }, ein: { type: 'string', description: 'company_setup: only if they gave it' }, pay_frequency: { type: 'string', description: 'company_setup: weekly, every two weeks, twice a month or monthly — only if they said' }, charges_sales_tax: { type: 'string', description: 'company_setup: "yes" or "no" only if they said' }, local_sales_tax_pct: { type: 'string', description: 'company_setup: the city/county add-on percent, only if they gave it' },
          merchant: { type: 'string', description: 'expense: who was paid, as printed on the receipt' }, receipt: { type: 'string', description: 'expense: "yes" when a receipt photo or PDF is attached to this conversation and this expense is from it' }, category: { type: 'string', description: 'expense: Fuel, Materials, Meals, Equipment Rental, Permits, Travel, Office Supplies, Subcontractor, Labor, Marketing, Insurance, Utilities, Cost of Sale or Other' },
          ...
          items: { type: 'array', description: 'price_book: one row per item read off the attached price list, photo, PDF or sheet — never invented, never rounded. Skip header/total rows.', items: { type: 'object', properties: { name: { type: 'string' }, unit_price: { type: 'number', description: 'the selling price per unit' }, cost: { type: 'number', description: 'what it costs the company, only if the document shows it' }, type: { type: 'string', description: 'Product or Service (labor, hourly, per-visit = Service)' }, description: { type: 'string' }, sku: { type: 'string' }, manufacturer: { type: 'string' }, model_number: { type: 'string' }, category: { type: 'string' } }, required: ['name', 'unit_price'] } }, source: { type: 'string', description: 'price_book: where the list came from, e.g. "Graybar price sheet" or the file name' },
        },
      },
